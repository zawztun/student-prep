...
export class StudentService {
  ...
  static async updateEmailPreference(
    ...
    const channels = emailPreference
      ? ["Email", "In-App"]
      : student.studyPlan.channels.filter(
...
export class WeeklyReportService {
  ...
  static async generateWeeklyReport(
    ...
    for (const assignment of assignments) {
      const { score } = await AssignmentSubmissionService.calculateScore(
        assignment.id
      );
      totalScore += score;
    ...
    Object.entries(subjectBreakdown).forEach(([subject, data]) => {
      ...
      if (percentage < 70) {
        ...
        achievements.push(
          `Excellent performance in ${subject}: ${percentage.toFixed(
            1
          )}% accuracy`
        );
      }
    ...
    if (averageScore >= 90) {
      achievements.push(
        `Outstanding overall performance: ${averageScore.toFixed(
          1
        )}% average score`
      );
    }
    ...
    if (assignmentsCompleted >= 5) {
      achievements.push(
        `Consistent study habit: Completed ${assignmentsCompleted} assignments this week`
      );
    }
  ...
  static async getStudentStats(studentId: string): Promise<{
    ...
    for (const assignment of assignments.filter(
      ...
      for (const submission of assignment.submissions) {
        ...
        const correctChoice = submission.question.choices.find(
          (c: QuestionChoice) => c.isCorrect
        );
        const isCorrect =
          correctChoice && correctChoice.id === submission.selectedChoiceId;
    ...
    const averageScore =
      completedAssignments > 0 ? totalScore / completedAssignments : 0;
